...
Plots.plot!(p2, z_range, pgf_safe(dA_exp(0.005, z_range, dA_exact, dA_z_exact, dA_zz_exact, dA_zzz_exact) ./ dA_exact_safe .- 1, limit=100.0), label=L"z_*=0.005", ls=:dot, color=line_colors[3])
Plots.plot!(p2, z_range, pgf_safe(dA_exp(0.010, z_range, dA_exact, dA_z_exact, dA_zz_exact, dA_zzz_exact) ./ dA_exact_safe .- 1, limit=100.0), label=L"z_*=0.010", ls=:dashdot, color=line_colors[4])
Plots.plot!(p2, z_range, pgf_safe(dA_exp(0.015, z_range, dA_exact, dA_z_exact, dA_zz_exact, dA_zzz_exact) ./ dA_exact_safe .- 1, limit=100.0), label=L"z_*=0.015", ls=:dashdotdot, color=line_colors[5])
Plots.xlabel!(p2, L"z")
...
anchor_z_vals = [0.000, 0.007, 0.013, 0.019] 

# Create a dedicated directory so they don't get mixed in with the standard z_ folders
target_dir = joinpath(BASE_PLOT_DIR, "expansion_errors_at_z0.010")
